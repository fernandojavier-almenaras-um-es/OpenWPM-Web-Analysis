...
if [ "$1" != "--skip-create" ]; then
  ...
  Darwin)
    ...
    CONDA_SUBDIR=osx-64 PYTHONNOUSERSITE=True mamba env create --yes -q -f environment.yaml # if you are using an old installation of Conda, use --force instead of --yes
    ;;
  *)
    PYTHONNOUSERSITE=True mamba env create --yes -q -f environment.yaml # if you are using an old installation of Conda, use --force instead of --yes
    ;;
  esac
