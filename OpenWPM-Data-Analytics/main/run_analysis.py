...
from utils.into_table_utils import analyzerObjects_to_dataframe, dataframe_to_table,table_exists, table_to_dataframe, merge_dataframes, PROTECTED_TABLE_NAMES # type: ignore
...
from utils.analyzers_utils import all_analyzers, all_dynamic_analyzers, all_static_analyzers, all_custom_analyzers, analyzers_from_class_names, run_analyzers, analyzers_from_module_names
...
if __name__ == '__main__':
    ...
    SingletonEngine.set_engine(engine)
...
The table names used by OpenWPM:\n{PROTECTED_TABLE_NAMES}""")
    ...
    if args.analyzers != "":
        match args.analyzers:
            case "dynamic":
                analyzer_objects: List[Analyzer] = all_dynamic_analyzers(db,logger)
            case "static":
                analyzer_objects: List[Analyzer] = all_static_analyzers(db,logger)
            case "custom":
                analyzer_objects: List[Analyzer] = all_custom_analyzers(db,logger)
            case _:
                analyzer_objects: List[Analyzer] = analyzers_from_module_names(str(args.analyzers).split(','), db,logger)
    elif args.analyzers_names != None:
        analyzer_objects: List[Analyzer] = analyzers_from_class_names(args.analyzers_names, db,logger)
    else:
        analyzer_objects: List[Analyzer] = all_analyzers(db,logger)
