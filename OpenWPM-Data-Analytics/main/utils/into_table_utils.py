...
def dataframe_to_table(df : pandas.DataFrame,engine : Engine, table_name : str) -> None:
     df.to_sql(table_name,engine,if_exists="replace",index_label=["visit_id","script_url","top_level_url"] )
...
def table_to_dataframe(engine : Engine, table_name : str) -> pandas.DataFrame:
    with engine.connect() as conn:
        return pandas.read_sql_table(table_name,conn,index_col=["visit_id","script_url","top_level_url"])


def analyzerObjects_to_dataframe(analyzer_objects : List[Analyzer]) -> pandas.DataFrame:
    domain : Set[Tuple[str,str,str]] = set()
    ...
    df: pandas.DataFrame = pandas.DataFrame(data=numpy.nan, index=pandas.MultiIndex.from_tuples(list(domain),names=['visit_id', 'script_url', 'top_level_url']), columns=analysis_names, dtype=float) #type: ignore
    for analyzer in analyzer_objects:
        ...
        for value in analyzer.get_analysis_results():
            df.at[value[:-1], analyzer.analysis_name()] = float(value[-1]) #type: ignore
    return df
