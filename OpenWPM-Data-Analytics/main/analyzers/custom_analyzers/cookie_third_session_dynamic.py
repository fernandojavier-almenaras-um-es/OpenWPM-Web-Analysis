...
class CookieThirdPartySession(Cookie_Dynamic_Analyzer):
    
    @staticmethod
    def fingerprinting_type() -> str:
        return "Cookie ThirdParty Session"
    ...
    def _read_row(self, row : Any) -> None:
        ...
        try:
            if not isFirstParty(row["script_url"], row["top_level_url"]) and row["is_session"] == 1:
